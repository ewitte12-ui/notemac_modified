 ...
 * Rejects non-HTTP(S) protocols and private/internal IP addresses.
 */
function ValidateBaseUrl(url: string): void
{
    ...
    if ('https:' !== parsed.protocol && 'http:' !== parsed.protocol)
        throw new Error(`Provider URL must use HTTP or HTTPS protocol (got "${parsed.protocol}")`);

    const hostname = parsed.hostname.toLowerCase();

    // Reject private/internal IP ranges and loopback
    ...
    for (const pattern of privatePatterns)
    {
        if (pattern.test(hostname))
            throw new Error(`Provider URL cannot target a private or internal address: "${hostname}"`);
    }

    if ('http:' === parsed.protocol)
        console.warn(`[LLMController] Warning: provider URL uses plain HTTP — HTTPS is strongly recommended: ${url}`);
}
...
): Promise<{ id: string; name: string; providerId: string; contextWindow: number; supportsStreaming: boolean; supportsFIM: boolean }[] | null>
{
    try
    {
        ...
        if ('google' === provider.type)
        {
            // Use x-goog-api-key header instead of URL query param to avoid
            // leaking the key in browser history, server logs, and referrer headers.
            const response = await fetch(
                `${provider.baseUrl}/v1beta/models?pageSize=100`,
