...
export async function SendChatMessage(userContent: string): Promise<void>
{
    ...
    // After the null-check above, conversationId is guaranteed non-null.
    const resolvedConversationId: string = conversationId;

    // Add placeholder assistant message
    const assistantMsg = CreateAssistantMessage();
    store.AddMessageToConversation(resolvedConversationId, assistantMsg);
    ...
    try
    {
        await SendChatCompletion(
            ...
            (chunk) =>
            {
                ...
                store.UpdateLastMessage(resolvedConversationId, fullResponse);
                Dispatch(NOTEMAC_EVENTS.AI_STREAM_CHUNK, { text: chunk });
            },
            (finalText) =>
            {
                // Update with final parsed code blocks
                store.UpdateLastMessage(resolvedConversationId, finalText);
                Dispatch(NOTEMAC_EVENTS.AI_RESPONSE_COMPLETE, { conversationId: resolvedConversationId, text: finalText });
            },
            (error) =>
            {
