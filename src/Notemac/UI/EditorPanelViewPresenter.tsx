...
import { defineCustomMonacoTheme } from '../Configs/ThemeConfig';
...
export function EditorPanel({ tab, theme, settings, zoomLevel }: EditorPanelProps) {
  ...
  // Update theme when it changes (including editor background for custom themes)
  useEffect(() => {
    if (!monacoRef.current) return;
    if ('notemac-custom' === theme.editorMonacoTheme && theme.customMonacoBase)
      defineCustomMonacoTheme(monacoRef.current, theme.customMonacoBase, theme.editorBg);
    monacoRef.current.editor.setTheme(theme.editorMonacoTheme);
  }, [theme.editorMonacoTheme, theme.editorBg, theme.customMonacoBase]);
