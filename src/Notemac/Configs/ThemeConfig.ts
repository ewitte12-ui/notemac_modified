export interface ThemeColors
{
    ...
    /** Only set for custom themes. Holds the base Monaco theme name to inherit rules/colors from. */
    customMonacoBase?: string;
    findBg: string;
}
...
export function GetCustomTheme(baseName: string, overrides: Partial<ThemeColors>): ThemeColors
{
    ...
    return {
        ...base,
        ...overrides,
        editorMonacoTheme: 'notemac-custom',
        customMonacoBase: base.editorMonacoTheme,
    };
}
...
// ─── Monaco theme data ────────────────────────────────────────────────────────
//
// Stored separately so defineCustomMonacoTheme can look up rules/colors for any
// base theme and re-define 'notemac-custom' with only editor.background swapped.
...
const monacoThemeDefs: Record<string, MonacoThemeDef> = {
    'mac-glass': {
        base: 'vs-dark',
        rules: [
...
{
    for (const [name, def] of Object.entries(monacoThemeDefs))
        monaco.editor.defineTheme(name, { base: def.base, inherit: true, rules: def.rules, colors: def.colors });
}

/**
 * Define (or redefine) the 'notemac-custom' Monaco theme by inheriting all
 * syntax rules and UI colors from baseMonacoThemeName, but replacing
 * editor.background with the provided editorBg value.
 *
 * Call this before monaco.editor.setTheme('notemac-custom') whenever the
 * custom theme's editorBg changes.
 */
export function defineCustomMonacoTheme(
    monaco: typeof import('monaco-editor'),
    baseMonacoThemeName: string,
    editorBg: string,
): void
{
    const def = monacoThemeDefs[baseMonacoThemeName];
    const base: 'vs-dark' | 'vs' = def?.base ?? (baseMonacoThemeName === 'vs' ? 'vs' : 'vs-dark');
    const rules = def?.rules ?? [];
    const colors = { ...(def?.colors ?? {}), 'editor.background': editorBg };
    monaco.editor.defineTheme('notemac-custom', { base, inherit: true, rules, colors });
}
