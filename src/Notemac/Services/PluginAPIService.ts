...
/** Maximum serialized size (bytes) a plugin may store per key (1 MB). */
const MAX_PLUGIN_STORAGE_VALUE_BYTES = 1024 * 1024;
...
function CreateStorageAPI(pluginId: string): PluginStorageAPI
{
    ...
    return {
        Get: (key: string) =>
        {
            ...
            {
                console.warn(`[PluginStorage:${pluginId}] Failed to read/parse key "${key}":`, e);
                return undefined;
            }
        },
        ...
        Set: (key: string, value: unknown) =>
        {
            try
            {
                ...
                if (serialized.length > MAX_PLUGIN_STORAGE_VALUE_BYTES)
                {
                    console.warn(
                        `[PluginStorage:${pluginId}] Value for key "${key}" exceeds ` +
                        `${MAX_PLUGIN_STORAGE_VALUE_BYTES / 1024} KB limit — write rejected`,
                    );
                    return;
                }
            ...
            {
                console.warn(`[PluginStorage:${pluginId}] Failed to write key "${key}":`, e);
            }
        },
    };
}
