...
/**
 * Validate that a plugin download URL is safe to fetch:
 * must be HTTPS and must share the same origin as the registry.
 */
function ValidateDownloadUrl(downloadUrl: string, registryUrl: string): void
{
    let parsed: URL;
    try
    {
    ...
    {
        throw new Error(`Invalid plugin download URL: "${downloadUrl}"`);
    }

    if ('https:' !== parsed.protocol)
        throw new Error(`Plugin download URL must use HTTPS (got "${parsed.protocol}")`);
    ...
    if (parsed.origin !== registryOrigin)
        throw new Error(
            `Plugin download URL origin "${parsed.origin}" does not match registry origin "${registryOrigin}"`,
        );
}
 ...
 * @param registryUrl - The URL of the registry this entry came from; used to
 *   validate that the download URL shares the same origin.
 */
...
): Promise<PluginManifest | null>
{
    try
    {
        // Validate download URL before fetching to prevent supply-chain attacks
        ValidateDownloadUrl(entry.downloadUrl, registryUrl);
